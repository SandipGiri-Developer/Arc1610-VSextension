...
const getUriDescription = (a: any, b: any) => ({ extension: "ts", relativePathOrBasename: "file.ts" });
  ...
  for (const p of editorState?.content || []) {
    switch (p.type) {
      ...
      case CodeBlock.name:
        ...
        const codeText = `\n\`\`\`${extension} ${relativePathOrBasename} (${rif.range?.start?.line || 0 + 1}-${rif.range?.end?.line || 0 + 1})\n${contextItem.content}\n\`\`\`\n`;
