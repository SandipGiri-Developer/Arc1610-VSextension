...
export default function StepContainer(props: StepContainerProps) {
  ...
  useEffect(() => {
    if (!isStreaming) {
      const content = (typeof props.item.message.content === "string" ? props.item.message.content : JSON.stringify(props.item.message.content)).trim();
      ...
      >
        {uiConfig?.displayRawMarkdown ? (
          <pre className="text-2xs max-w-full overflow-x-auto whitespace-pre-wrap break-words p-4">
            {(typeof props.item.message.content === "string" ? props.item.message.content : JSON.stringify(props.item.message.content))}
          </pre>
        ) : (
          <>
            ...
            <StyledMarkdownPreview
              ...
              source={props.item.message.content as any}
